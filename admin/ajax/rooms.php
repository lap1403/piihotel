<?php  
    ...
    if(isset($_POST['get_all_rooms']))
    {
        ...
        while ($row = mysqli_fetch_assoc($res)) {
            if($row['status']==1){
                $status="<button onclick='toggle_status($row[id],0)' class='btn btn-dark btm-sm shadow-none'>active</button>";
            }
            else{
                $status="<button onclick='toggle_status($row[id],1)'  class='btn btn-warning btm-sm shadow-none'>inactive</button>";
            }
    ...
    if(isset($_POST['edit_room']))
    {
        ...
        $del_features =del("DELETE FROM `room_features` WHERE `room_id`=?" ,[$frm_data['room_id']],'i');
    ...
    if(isset($_POST['get_room_images']))
    {
        ...
        {
            if($row['thumb']==1){
            $thumb_btn="<i class='bi bi-check-lg text-lign bg-success px-2 py-1 rounded fs-5'></i>";
            }
            else{
                $thumb_btn=" <button onclick='thumb_image($row[sr_no],$row[room_id])' class='btn btn-secondary btm-sm shadow-none'>
            ...
            echo<<<data
                <tr class='align-middle'>
                    ...
                    <td>$thumb_btn</td>
                    <td>
                     <button onclick='rem_image($row[sr_no],$row[room_id])' class='btn btn-danger btm-sm shadow-none'>
                        <i class='bi bi-trash'></i>
                     </button>
                    </td>
                </tr>
    ...
    if(isset($_POST['remove_room']))
    {
        ...
        $res2=del("DELETE FROM `room_images` WHERE `room_id`=?",[$frm_data['room_id']],'i');
        $res3=del("DELETE FROM `room_features` WHERE `room_id`=?",[$frm_data['room_id']],'i');
        $res4=del("DELETE FROM `room_facilities` WHERE `room_id`=?",[$frm_data['room_id']],'i');
